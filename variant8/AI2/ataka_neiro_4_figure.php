...
/**
 * Функция глубокого анализа ходов медведя на 4 шага вперед
 */
function evaluateMedvedChainPHP52($startIndex, $board, $depth, $color) {
    $directions = array(
        array(-2, 2, -48, 48, -46, 46, -49, 49, -50, 50, -22, 22, -47, 46, -26, 26)
    );
    ...
    foreach ($directions as $line) {
        foreach ($line as $offset) {
            ...
            $nextSteps = evaluateMedvedChainPHP52($targetIndex, $tempBoard, $depth - 1, $color);
            $totalScore = $currentScore + $nextSteps['score'];
...
/**
 * Функция глубокого анализа ходов ястреба на 4 шага вперед
 */
function evaluateYastrebChainPHP52($startIndex, $board, $depth, $color) {
    $directions = array(
        array(-75, 75, -69, 69, -48, 48, -72, 72, -2, 2, -3, 3, -50, 50, -46, 46)
    );
    ...
    foreach ($directions as $line) {
        foreach ($line as $offset) {
            ...
            $nextSteps = evaluateYastrebChainPHP52($targetIndex, $tempBoard, $depth - 1, $color);
            $totalScore = $currentScore + $nextSteps['score'];
...
/**
 * Универсальная функция глубокого анализа ходов Призрака на 4 шага вперед
 * Совместима с PHP 5.2*/

function evaluateGhostChainPHP52($startIndex, $board, $depth, $color) {
    // Смещения для ДВИЖЕНИЯ призрака (на пустые клетки)
    $moveOffsets = array(
        2, -2, 3, -3, 48, -48, 72, -72, 26, -26, 27, -27, 
    ...
    foreach ($allOffsets as $offset) {
        ...
        // Рекурсивный вызов
        $nextSteps = evaluateGhostChainPHP52($targetIndex, $tempBoard, $depth - 1, $color);
        $totalScore = $currentScore + $nextSteps['score'];
...
for($rer = 53; $rer < 150; $rer++) {
   ...
   // $index_to = null;
    ...
    // АТАКА ЧЕРНОГО МЕДВЕДЯ (ID: 8)
    if(isset($sh[$index]) && $sh[$index] == 8 && isset($exxo[4]) && $exxo[4] == 'Comp' && isset($exxo[11]) && $exxo[11] == 'Черные' && !$index_to) {
        $analysis = evaluateMedvedChainPHP52($index, $sh, $dept, 'Черные');
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Выбрана стратегическая цель для атаки черного медведя: ' . $index_to . ' (Прогноз ценности на 4 хода: ' . $analysis['score'] . ')<br>';
        }
        if($index_to) {
            $hod_one = $index;
            $hod_two = $index_to;
    ...
    // АТАКА БЕЛОГО МЕДВЕДЯ (ID: 18)
    if(isset($sh[$index]) && $sh[$index] == 18 && isset($exxo[3]) && $exxo[3] == 'Comp' && isset($exxo[11]) && $exxo[11] == 'Белые' && !$index_to) {
        $analysis = evaluateMedvedChainPHP52($index, $sh, $dept, 'Белые');
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Выбрана стратегическая цель для атаки белого медведя: ' . $index_to . ' (Прогноз ценности на 4 хода: ' . $analysis['score'] . ')<br>';
        }
        if($index_to) {
            $hod_one = $index;
            $hod_two = $index_to;
    ...
    // АТАКА ЧЕРНОГО ЯСТРЕБА (ID: 7)
    if(isset($sh[$index]) && $sh[$index] == 7 && isset($exxo[4]) && $exxo[4] == 'Comp' && isset($exxo[11]) && $exxo[11] == 'Черные' && !$index_to) {
        $analysis = evaluateYastrebChainPHP52($index, $sh, $dept, 'Черные');
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Выбрана стратегическая цель для атаки черного ястреба: ' . $index_to . ' (Прогноз ценности на 4 хода: ' . $analysis['score'] . ')<br>';
        }
        if($index_to) {
            $hod_one = $index;
            $hod_two = $index_to;
    ...
    // АТАКА БЕЛОГО ЯСТРЕБА (ID: 17)
    if(isset($sh[$index]) && $sh[$index] == 17 && isset($exxo[3]) && $exxo[3] == 'Comp' && isset($exxo[11]) && $exxo[11] == 'Белые' && !$index_to) {
        $analysis = evaluateYastrebChainPHP52($index, $sh, $dept, 'Белые');
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Выбрана стратегическая цель для атаки белого ястреба: ' . $index_to . ' (Прогноз ценности на 4 хода: ' . $analysis['score'] . ')<br>';
        }
        if($index_to) {
            $hod_one = $index;
            $hod_two = $index_to;
...
// 1. АТАКА/ХОД ЧЕРНОГО ПРИЗРАКА
    if($sh[$index] == 9 && $exxo[4] == 'Comp' && $exxo[11] == 'Черные' && !$index_to) { 
        
        $analysis = evaluateGhostChainPHP52($index, $sh, $dept, 'Черные');
        
        // Исправлено условие: ходим, даже если score == 0 (нет немедленной жертвы, просто маневр)
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Призрак (Ч) перемещается/атакует клетку: ' . $index_to . ' (Прогноз ценности: ' . $analysis['score'] . ')<br>';
        }
    ...
    // 2. АТАКА/ХОД БЕЛОГО ПРИЗРАКА
    if($sh[$index] == 19 && $exxo[3] == 'Comp' && $exxo[11] == 'Белые' && !$index_to) { 
        
        $analysis = evaluateGhostChainPHP52($index, $sh, $dept, 'Белые');
        
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Призрак (Б) перемещается/атакует клетку: ' . $index_to . ' (Прогноз ценности: ' . $analysis['score'] . ')<br>';
        }
...
if($b1==0 && $hod_two<1000) { $hod_one=1000; $hod_two=1000; } else
