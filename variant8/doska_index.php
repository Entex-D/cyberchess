...
if(!$nomer8 && isset($_COOKIE['nomer_doski8'])) { //echo '<br>считан номер доски='.$nomer_doski8;
  ...
  if($game!='5' && $game>'0' && !isset($_COOKIE['nomer_doski8'])) {
  
   include('./ochki/vsego_igr_write.php'); 

$fx=rand(1,10000); $nomer_doski8='a'.$fx.'h'; 
if(setcookie ("nomer_doski8", $nomer_doski8, time() + 3600*24*700)) { //echo '<br>куки не записались'; 
} 
                                                               }
...
if(isset($_COOKIE['doska_kuki_nomer8']))  { $doska=$doska_kuki_nomer8; }
...
fclose ($file);
              ...
              if($game<'5' && $game>'0' && !isset($_COOKIE['nomer_doski8'])) { echo ''; 
...
if($name==$doska) { //$fx=rand(1,10000); $nomer_doski8='n'.$fx.'r'; 
//if(!setcookie ("nomer_doski8", $nomer_doski8, time() + 3600*24*700)) { echo '<br>куки не записались'; } 

$adres='./archive_doski/'.$name.'/tip8/'.$nomer_doski8;
...
//echo '<br>$doska='.$doska;
//echo '<br>$name='.$name;
//echo '<br>$nomer_doski8='.$nomer_doski8;

$figury='./archive_doski/'.$name.'/tip8/'.$nomer_doski8.'/'.$nomer_doski8.'.txt';
...
$dannye_archive='./archive_doski/'.$name.'/tip8/'.$nomer_doski8.'/'.$nomer_doski8.'_statix.txt';
...
$logi_text='<font size=1><em>'.date("d.m.Y|G:i:s ").'</em></font> Игрок <strong><em>'.$name.'</em></strong> создал  новую игру '.$game_g.' на доске '.$nomer_doski8.' вариант #8 Ставка '.$dixa20.' btc';
...
if($game=='5' && isset($_COOKIE['nomer_doski8'])) { echo ''; 

$logi='logi.txt';

$logi_text='<font size=1><em>'.date("d.m.Y|G:i:s ").'</em></font> Игрок <strong><em>'.$name.'</em></strong> отменил игру на доске '.$nomer_doski8.' вариант #8';
$openchatdb=fopen($logi,"a");
 if(fwrite($openchatdb,$logi_text."\n")) { ;  }
fclose($openchatdb);
...
//echo '$nomer_doski8='.$nomer_doski8;
...
<? if($name==$exxo[4] && $nomer_doski8 && $exxo[5]=='0' && $exxo[18]=='0') { echo '';  ?>
...
if($exxo[1]!='Comp') { echo '<br><a href=index.php?doska='.$doska.'&tip=8&nomer_doski8='.$nomer_doski8.'&ilost=yes>Сдаться</a>'; } 
 }}
...
<? if($name==$exxo[3] && $nomer_doski8 && $exxo[5]=='0' && $exxo[18]=='0') {  // если играем белыми - доска в своем положении
?>
...
echo '<br><a href=index.php?doska='.$doska.'&tip=8&nomer_doski8='.$nomer_doski8.'&ilost=yes>Сдаться</a>'; 
 }}
// конец обновление вместо "онлайн людей" от 15.06.2020
?>
...
<? 
      echo '<center><strong>Нептун 10x8</strong></center><br><br><font size=2><strong><center>';
?>

<a target=_blank href=model_game/variant8-igra1-style<? echo $style_run; ?>.jpg><img src=model_game/variant8-igra1-style<? echo $style_run; ?>.jpg width=30% /></a>

<? echo '<br><br><br>Создайте новую игру '.$name.'</center></strong></font></td>';
      //include('prosmotr_doski.php');
  } else if($nomer_doski8 && $name!=$exxo[3] && $name!=$exxo[4] && $exxo[18]=='0' && $exxo[1]!='Comp') {
 
      echo '<font size=4><strong><center>Доска '.$nomer_doski8.' занята,создайте свою игру '.$name.'</center></strong></font>';
      echo '<font size=2><strong><em><center>
Посмотреть партию <a target=_blank href=variant8/archive_d_tip4_uvel.php?namer='.$doska.'&tip=8&nomer8='.$nomer_doski8.'>в архиве</a>
</center></em></strong></font></td>';
      //include('prosmotr_doski.php'); 

} else if($exxo[18]=='Отменена') {

     echo '<font size=4><strong><center>Доска '.$nomer_doski8.' отменена,игроком '.$doska.'</center></strong></font>
               <font size=2><strong><em><center>Посмотреть доску '.$nomer_doski8.' <a target=_blank href=variant8/archive_d_tip4_uvel.php?namer='.$doska.'&tip=8&nomer8='.$nomer_doski8.'>в архиве</a></center></em></strong></font>
</td>';
    //  include('prosmotr_doski.php'); 

if($exxo[5]!='0' || $exxo[18]!='0') {  
 $nomer_doski8='';  setcookie ("nomer_doski8", $nomer_doski8, time() + 1);
$nomer8='';  setcookie ("nomer8", $nomer8, time() + 1); 
}
...
if($exxo[5]==$name) {
     echo '<font size=4><strong><center>Вы выиграли играя '.$d.', '.$bex.' проиграл</center></strong></font>';
               ...
               echo '<font size=2><strong><em><center>Посмотреть доску '.$nomer_doski8.' <a target=_blank href=variant8/archive_d_tip4_uvel.php?namer='.$doska.'&tip=8&nomer8='.$nomer_doski8.'>в архиве</a></center></em></strong></font>';
...
$data1='./archive_doski/'.$doska.'/tip8/'.$nomer_doski8.'/'.$nomer_doski8.'_hod_vartical_white.txt';
...
if($exxo[21]>'9' || $exxo[22]>'9') {
 include('./variant8/hod_vertical/hod_vertical.php'); 
                                                    }
}
...
$data1='./archive_doski/'.$doska.'/tip8/'.$nomer_doski8.'/'.$nomer_doski8.'_random_figura_white.txt';
...
if($exxo[21]>'9' || $exxo[22]>'9') {
 include('./variant8/random_figura/hod_vertical.php'); 
                                                    }
} 
 ...
 include('./variant8/hod_vertical/random_table.php'); 
...
if($zvuki_hody=='11') { 
 ...
 if($exxo[12]<2) { include('./variant8/zvuk_start_game.php'); } 
if($exxo[12]>1 && $exxo[12]!=10) { include('./variant8/zvuk.php'); }
