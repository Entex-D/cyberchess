...
echo '&nbsp;&nbsp;&nbsp;Досок '.$namer.' - варианта #6 найдено '.$doski_tip6.':&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';  
...
if($doski_tip6>$doski_v_archive) { $doski_tip6=$doski_v_archive; }

for($s=1;$s<$doski_tip6+2;$s++) {   //начало for

$ndoski='../archive_doski/'.$namer.'/tip6/'.$dir_file_cnt[$s].'/'.$dir_file_cnt[$s].'_hody.txt';
 $b4='0';
...
if($b4>'1') {
echo '&nbsp;&nbsp;&nbsp;<a target=_blank href=archive_d_tip4_uvel.php?namer='.$namer.'&tip=6&nomer6='. $dir_file_cnt[$s].'>Увеличить</a>';
                 } else if($b4<'2') { echo '&nbsp;&nbsp;&nbsp;<font size=1><strong>Не показано</strong></font>'; 
if($reset=='1') { echo '<font size=1>&nbsp;&nbsp;<a href=archive_doski_tip4.php?namer='.$namer.'&nomer6s='.$dir_file_cnt[$s].'&del=yes>Удалить</a></font>'; 
 } 
} // конец else
...
$fdoski='../archive_doski/'.$namer.'/tip6/'.$dir_file_cnt[$s].'/'.$dir_file_cnt[$s].'.txt';
 $b5='0';
...
$fstatix='../archive_doski/'.$namer.'/tip6/'.$dir_file_cnt[$s].'/'.$dir_file_cnt[$s].'_statix.txt';
 $b6='0';
...
if($namer==$name && $dstatix[5]=='0') { echo '&nbsp;&nbsp;&nbsp;<a target=_blank href=../index.php?doska='.$namer.'&tip=6&nomer6='. $dir_file_cnt[$s].'>Играть</a>'; }
if($namer!=$name && $dstatix[2]=='0') { echo '&nbsp;&nbsp;&nbsp;<a target=_blank href=../index.php?doska='.$namer.'&tip=6&nomer6='. $dir_file_cnt[$s].'>Стать соперником</a>'; }

if($dstatix[1]<' ' || $figury[1]<' ') {  $nomer6s=$dir_file_cnt[$s]; $del='yes';}
...
$doski_tip6_n1=count($dir_file_cnt_n);

echo '&nbsp;&nbsp;&nbsp;Игроков всего найдено '.$doski_tip6_n1.':<hr width=100% class=def size=2><br><center>';

$i='0';
for($s=0;$s<$doski_tip6_n1+3;$s++) {   //начало for

echo '&nbsp;&nbsp;&nbsp;<a href=archive_doski_tip4.php?namer='.$dir_file_cnt_n[$s].'&tip=6>'.$dir_file_cnt_n[$s].'</a>';
          $i=$i+'1';
if($i=='11') { $i='0'; echo '<br>'; }                         
}
echo '</center><br>'; 
...
//echo '<br><br>&nbsp;&nbsp;&nbsp;&nbsp;<a href=archive_doski_tip6.php?namer='.$namer.'&deske='.$deske.'>Следующие</a> 10 досок игрока '.$namer;
?>
...
$adres='../archive_doski/'.$namer.'/tip6/'.$nomer6s;

//echo '$nomer6s='.$nomer6s;
...
//echo "<meta http-equiv='refresh' content='300;url=../variant7/archive_doski_tip4.php?namer=$namer'>";
