...
if($sopernik) { if($nomer6 || $nomer_doski6) { unset($nomer6);  unset($nomer_doski6);
                              setcookie ("nomer6", $nomer6, time() );  setcookie ("nomer_doski6", $nomer_doski6, time() );         }
                   }
...
if(!file_exists($sxx))   {  //echo 'файл '.$sxx.' присутствует';
                          ...
                          setcookie ("nomer_doski6", $nomer_doski6, time() + 1);
                          setcookie ("nomer6", $nomer6, time() + 1);
                     ?>
<script>location.reload();</script>                     
<?
// echo "<meta http-equiv='refresh' content='1;url=index.php?doska=$name&tip6=$tip6'>";
}
...
if($file = fopen($base_hod_ip6, "r")) {   $b4=0;} 
while(!feof($file)) { 
...
fclose($openchatdb);
 ...
 $base_hod_ip6 = 'ip6/hod/'.$name.'.txt';

$openchatdb=fopen($base_hod_ip6,"w");
 if(fwrite($openchatdb,"")) { // echo 'Записано в файл '.$base_hod_ip6.'</font><strong></center><br>';
                                           }
fclose($openchatdb);
...
if($_POST['sbros']) { 
//$game='5'; 
//$nomer6_rezet=''; $nomer6=$nomer6_rezet;
setcookie ("game", $game, time() + 4);
 setcookie ("nomer_doski6", $nomer_doski6, time() + 1);  
 setcookie ("nomer6", $nomer6, time() + 1);  
...
setcookie ("game", $game, time() + 4);
 setcookie ("nomer_doski6", $nomer_doski6, time() + 1);  
// setcookie ("nomer6", $nomer6,4 time() + 1);  
 setcookie ("timer", $timer, time() + 1);  
setcookie ("btc", $btc, time() + 1);  
$rrt='4'; 
...
if($nomer6>' ') {  setcookie ("nomer6", $nomer6, time() +3600);  }

//если нет своей игры,и открываю свою игру из оповещений
if(!$nomer_doski6 && $nomer6) { $nomer_doski6=$nomer6; setcookie ("nomer_doski6", $nomer_doski6, time() +3600*24*700); }

//echo '<br>$nomer6='.$nomer6;
