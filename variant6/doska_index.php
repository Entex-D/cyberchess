...
if(!$nomer6 && isset($_COOKIE['nomer_doski6'])) { //echo '<br>считан номер доски='.$nomer_doski6;
  ...
  if($game!='5' && $game>'0' && !isset($_COOKIE['nomer_doski6'])) {
  
   include('./ochki/vsego_igr_write.php'); 

$fx=rand(1,10000); $nomer_doski6='a'.$fx.'f'; 
if(setcookie ("nomer_doski6", $nomer_doski6, time() + 3600*24*700)) { //echo '<br>куки не записались'; 
} 
                                                               }
...
if(isset($_COOKIE['doska_kuki_nomer6']))  { $doska=$doska_kuki_nomer6; }
...
include('./variant6/config_type4.php');
...
fclose ($file);
              ...
              if($game<'5' && $game>'0' && !isset($_COOKIE['nomer_doski6'])) { echo ''; 
...
if($name==$doska) { //$fx=rand(1,10000); $nomer_doski6='n'.$fx.'r'; 
//if(!setcookie ("nomer_doski6", $nomer_doski6, time() + 3600*24*700)) { echo '<br>куки не записались'; } 

$adres='./archive_doski/'.$name.'/tip6/'.$nomer_doski6;
...
//echo '<br>$doska='.$doska;
//echo '<br>$name='.$name;
//echo '<br>$nomer_doski6='.$nomer_doski6;

$figury='./archive_doski/'.$name.'/tip6/'.$nomer_doski6.'/'.$nomer_doski6.'.txt';
...
$dannye_archive='./archive_doski/'.$name.'/tip6/'.$nomer_doski6.'/'.$nomer_doski6.'_statix.txt';
...
$logi_text='<font size=1><em>'.date("d.m.Y|G:i:s ").'</em></font> Игрок <strong><em>'.$name.'</em></strong> создал  новую игру '.$game_g.' на доске '.$nomer_doski6.' вариант #6 Ставка '.$dixa20.' btc';
...
if($game=='5' && isset($_COOKIE['nomer_doski6'])) { echo ''; 

$logi='logi.txt';

$logi_text='<font size=1><em>'.date("d.m.Y|G:i:s ").'</em></font> Игрок <strong><em>'.$name.'</em></strong> отменил игру на доске '.$nomer_doski6.' вариант #6';
$openchatdb=fopen($logi,"a");
 if(fwrite($openchatdb,$logi_text."\n")) { ;  }
fclose($openchatdb);
...
//echo '$nomer_doski6='.$nomer_doski6;
...
// считываем типа $nomer6 и перезаписываем игрока 2 в файл,чтобы показалась доска для 2 игрока
...
// конец считываем типа $nomer6 и перезаписываем игрока 2 в файл,чтобы показалась доска для 2 игрока
?>
...
<? if($name==$exxo[4] && $nomer_doski6 && $exxo[5]=='0' && $exxo[18]=='0') { echo '';  ?>
...
if($exxo[1]!='Comp') { echo '<br><a href=index.php?doska='.$doska.'&tip=6&nomer_doski6='.$nomer_doski6.'&ilost=yes>Сдаться</a>'; } 
 }}
...
<? if($name==$exxo[3] && $nomer_doski6 && $exxo[5]=='0' && $exxo[18]=='0') {  // если играем белыми - доска в своем положении
?>
...
echo '<br><a href=index.php?doska='.$doska.'&tip=6&nomer_doski6='.$nomer_doski6.'&ilost=yes>Сдаться</a>'; 
 }}
// конец обновление вместо "онлайн людей" от 15.06.2020
?>
...
<a target=_blank href=model_game/variant6-igra1-style<? echo $style_run; ?>.jpg><img src=model_game/variant6-igra1-style<? echo $style_run; ?>.jpg width=30% /></a>

<? echo '<br><br><br>Создайте новую игру '.$name.'</center></strong></font></td>';
      //include('prosmotr_doski.php');
  } else if($nomer_doski6 && $name!=$exxo[3] && $name!=$exxo[4] && $exxo[18]=='0' && $exxo[1]!='Comp') {
 
      echo '<font size=4><strong><center>Доска '.$nomer_doski6.' занята,создайте свою игру '.$name.'</center></strong></font>';
      echo '<font size=2><strong><em><center>
Посмотреть партию <a target=_blank href=variant6/archive_d_tip4_uvel.php?namer='.$doska.'&tip=6&nomer6='.$nomer_doski6.'>в архиве</a>
</center></em></strong></font></td>';
      //include('prosmotr_doski.php'); 

} else if($exxo[18]=='Отменена') {

     echo '<font size=4><strong><center>Доска '.$nomer_doski6.' отменена,игроком '.$doska.'</center></strong></font>
               <font size=2><strong><em><center>Посмотреть доску '.$nomer_doski6.' <a target=_blank href=variant6/archive_d_tip4_uvel.php?namer='.$doska.'&tip=6&nomer6='.$nomer_doski6.'>в архиве</a></center></em></strong></font>
</td>';
    //  include('prosmotr_doski.php'); 

if($exxo[5]!='0' || $exxo[18]!='0') {  
 $nomer_doski6='';  setcookie ("nomer_doski6", $nomer_doski6, time() + 1);
$nomer6='';  setcookie ("nomer6", $nomer6, time() + 1); 
}
...
if($exxo[5]==$name) {
     echo '<font size=4><strong><center>Вы выиграли играя '.$d.', '.$bex.' проиграл</center></strong></font>';
               ...
               echo '<font size=2><strong><em><center>Посмотреть доску '.$nomer_doski6.' <a target=_blank href=variant6/archive_d_tip4_uvel.php?namer='.$doska.'&tip6=6&nomer6='.$nomer_doski6.'>в архиве</a></center></em></strong></font>';
...
<tr>
<td bgcolor=<? echo $color_x; ?>><center>Гений</center></td>
<td bgcolor=<? echo $color_x; ?>><center><? echo $figure_dx8_coord; ?> <? echo $figure_dx8x_coord; ?></center></td>
<td bgcolor=<? echo $color_x; ?>><center><a target=_blank href=../model_game/<? echo $fixxa; ?>pravila_geniy.jpg>Правила ходов у Гения</a></center></td>
</tr>

<tr>
<td bgcolor=<? echo $color_x; ?>><center>Insider</center></td>
<td bgcolor=<? echo $color_x; ?>><center><? echo $figure_dx7_coord; ?> <? echo $figure_dx7x_coord; ?></center></td>
<td bgcolor=<? echo $color_x; ?>><center><a target=_blank href=../model_game/<? echo $fixxa; ?>pravila_insider.jpg>Правила ходов у Insider</a></center></td>
</tr>

<tr>
<td bgcolor=<? echo $color_x; ?>><center>Паук</center></td>
<td bgcolor=<? echo $color_x; ?>><center><? echo $figure_dx9_coord; ?> <? echo $figure_dx9x_coord; ?></center></td>
<td bgcolor=<? echo $color_x; ?>><center><a target=_blank href=../model_game/<? echo $fixxa; ?>pravila_pauk.jpg>Правила ходов у Паука</a></center></td>
</tr>
...
$data1='./archive_doski/'.$doska.'/tip6/'.$nomer_doski6.'/'.$nomer_doski6.'_hod_vartical_white.txt';
 
if ( !file_exists($data1) ) { 

 include('./variant6/hod_vertical/hod_vertical_create.php'); 

}

include('./variant6/hod_vertical/hod_vertical_off.php'); ?>
<? if($exxo[11] && $shah!='1' && $shah!='2' && $shah_off[0]!='1' && $shah_off[1]=='1') { 


if($exxo[21]>'9' || $exxo[22]>'9') {
 include('./variant6/hod_vertical/hod_vertical.php'); 
                                                    }
}
...
$data1='./archive_doski/'.$doska.'/tip6/'.$nomer_doski6.'/'.$nomer_doski6.'_random_figura_white.txt';
 
if ( !file_exists($data1) ) { 

 include('./variant6/random_figura/hod_vertical_create.php'); 
...
if($exxo[21]>'9' || $exxo[22]>'9') {
 include('./variant6/random_figura/hod_vertical.php'); 
                                                    }
} 
 ...
 include('./variant6/hod_vertical/random_table.php'); 
...
<?
 if($exxo[12]<2) { include('./variant6/zvuk_start_game.php'); } 
if($exxo[12]>1 && $exxo[12]!=11) { include('./variant6/zvuk.php'); }
