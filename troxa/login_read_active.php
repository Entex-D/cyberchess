...
if($login>' ') {
 echo "<meta http-equiv='refresh' content='1;url=troxa.php?ban=ok3&login=$login'>";
                   }
}

if($ban_n>' ' || $offban_n>' ' || $del>' ') {
 echo "<meta http-equiv='refresh' content='1;url=troxa.php?ban=ok3'>";
                   }
...
$data4='../btc/'.$file.'.txt';
if ( !file_exists($data4) )   { $delete_total='Ok'; }
...
if($name==$name_admin && preg_match('/'.$ip_adress.'/',$ip) && $desa==$desa_parol) { 
                                                                    if($ban_login==false) {
                                                                             $dede1='<a href=troxa.php?ban=ok3&ban_n='.$file.'&v='.$x.'>Забанить</a>'; 
                                                                             $dede2='* * *';
                                                                             $dede3='<a href=troxa.php?ban=ok3&del='.$file.'>Удалить</a>';
 } 
                                                                                                      }
if($name==$name_admin && preg_match('/'.$ip_adress.'/',$ip) && $desa==$desa_parol) { 
                                                             if($ban_login==true) {
                                                                             $dede1='Забанен '; 
                                                                             $dede2='<a href=troxa.php?ban=ok3&offban_n='.$file.'>Разбанить</a>'; 
                                                                             $dede3='<a href=troxa.php?ban=ok3&del='.$file.'>Удалить</a>'; }
