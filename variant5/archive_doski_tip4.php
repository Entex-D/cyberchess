...
echo '&nbsp;&nbsp;&nbsp;Досок '.$namer.' - варианта #5 найдено '.$doski_tip5.':&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';  
...
if($doski_tip5>$doski_v_archive) { $doski_tip5=$doski_v_archive; }

for($s=1;$s<$doski_tip5+2;$s++) {   //начало for

$ndoski='../archive_doski/'.$namer.'/tip5/'.$dir_file_cnt[$s].'/'.$dir_file_cnt[$s].'_hody.txt';
 $b4='0';
...
if($b4>'1') {
echo '&nbsp;&nbsp;&nbsp;<a target=_blank href=archive_d_tip4_uvel.php?namer='.$namer.'&tip=5&nomer5='. $dir_file_cnt[$s].'>Увеличить</a>';
                 } else if($b4<'2') { echo '&nbsp;&nbsp;&nbsp;<font size=1><strong>Не показано</strong></font>'; 
if($reset=='1') { echo '<font size=1>&nbsp;&nbsp;<a href=archive_doski_tip4.php?namer='.$namer.'&nomer5s='.$dir_file_cnt[$s].'&del=yes>Удалить</a></font>'; 
 } 
} // конец else
...
$fdoski='../archive_doski/'.$namer.'/tip5/'.$dir_file_cnt[$s].'/'.$dir_file_cnt[$s].'.txt';
 $b5='0';
...
$fstatix='../archive_doski/'.$namer.'/tip5/'.$dir_file_cnt[$s].'/'.$dir_file_cnt[$s].'_statix.txt';
 $b6='0';
...
if($namer==$name && $dstatix[5]=='0') { echo '&nbsp;&nbsp;&nbsp;<a target=_blank href=../index.php?doska='.$namer.'&tip=5&nomer5='. $dir_file_cnt[$s].'>Играть</a>'; }
if($namer!=$name && $dstatix[2]=='0') { echo '&nbsp;&nbsp;&nbsp;<a target=_blank href=../index.php?doska='.$namer.'&tip=5&nomer5='. $dir_file_cnt[$s].'>Стать соперником</a>'; }

if($dstatix[1]<' ' || $figury[1]<' ') {  $nomer5s=$dir_file_cnt[$s]; $del='yes';}
...
$adres='../archive_doski/'.$namer.'/tip5/'.$nomer5s;

//echo '$nomer5s='.$nomer5s;
...
//echo "<meta http-equiv='refresh' content='300;url=../variant6/archive_doski_tip4.php?namer=$namer'>";
