...
/**
 * Универсальная функция глубокого анализа ходов Дракона на 4 шага вперед
...
function evaluateGhost7ChainPHP52($startIndex, $board, $depth, $color) {
    ...
    $moveOffsets = array(
        22, -22, 23, -23, 25, -25, 26, -26, 47, -47, 49, -49
    );

    // Смещения для АТАКИ призрака (только если там враг)
    $attackOffsets = array(1, -1, 24, -24, 2, -2, 48, -48);
...
/**
 * Универсальная функция глубокого анализа ходов Стрела на 4 шага вперед
 * Совместима с PHP 5.2*/

function evaluateGhost8ChainPHP52($startIndex, $board, $depth, $color) {
    // Смещения для ДВИЖЕНИЯ призрака (на пустые клетки)
    $moveOffsets = array(
        2, -2, 48, -48, 46, -46, 50, -50
    );

    // Смещения для АТАКИ призрака (только если там враг)
    $attackOffsets = array(3, -3, 22, -22, 26, -26, 47, -47, 49, -49, 72, -72);
...
/**
 * Универсальная функция глубокого анализа ходов Отшельника на 4 шага вперед
 * Совместима с PHP 5.2*/

function evaluateGhost9ChainPHP52($startIndex, $board, $depth, $color) {
    // Смещения для ДВИЖЕНИЯ призрака (на пустые клетки)
    $moveOffsets = array(
        23, -23,25, -25, 46, -46, 50, -50
    );

    // Смещения для АТАКИ призрака (только если там враг)
    $attackOffsets = array(22, -22, 26, -26, 47, -47, 49, -49);
...
// 1. АТАКА/ХОД ЧЕРНОГО ДРАКОНА
    if($sh[$index] == 7 && $exxo[4] == 'Comp' && $exxo[11] == 'Черные' && !$index_to) { 
        ...
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Дракон (Ч) перемещается/атакует клетку: ' . $index_to . ' (Прогноз ценности: ' . $analysis['score'] . ')<br>';
        }
    ...
    if($sh[$index] == 17 && $exxo[3] == 'Comp' && $exxo[11] == 'Белые' && !$index_to) { 
        ...
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Дракон (Б) перемещается/атакует клетку: ' . $index_to . ' (Прогноз ценности: ' . $analysis['score'] . ')<br>';
        }
...
// 1. АТАКА/ХОД ЧЕРНОЙ СТРЕЛЫ
    if($sh[$index] == 8 && $exxo[4] == 'Comp' && $exxo[11] == 'Черные' && !$index_to) { 
        ...
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Стрела (Ч) перемещается/атакует клетку: ' . $index_to . ' (Прогноз ценности: ' . $analysis['score'] . ')<br>';
        }
    ...
    if($sh[$index] == 18 && $exxo[3] == 'Comp' && $exxo[11] == 'Белые' && !$index_to) { 
        ...
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Стрела (Б) перемещается/атакует клетку: ' . $index_to . ' (Прогноз ценности: ' . $analysis['score'] . ')<br>';
        }
...
// 1. АТАКА/ХОД ЧЕРНОГО ОТШЕЛЬНИКА
    if($sh[$index] == 9 && $exxo[4] == 'Comp' && $exxo[11] == 'Черные' && !$index_to) { 
        ...
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Отшельник (Ч) перемещается/атакует клетку: ' . $index_to . ' (Прогноз ценности: ' . $analysis['score'] . ')<br>';
        }
    ...
    if($sh[$index] == 19 && $exxo[3] == 'Comp' && $exxo[11] == 'Белые' && !$index_to) { 
        ...
        if($analysis['best_move'] !== null && $analysis['score'] > 0) {
            $index_to = $analysis['best_move'];
            echo 'Отшельник (Б) перемещается/атакует клетку: ' . $index_to . ' (Прогноз ценности: ' . $analysis['score'] . ')<br>';
        }
