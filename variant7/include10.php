...
if($sopernik) { if($nomer7 || $nomer_doski7) { unset($nomer7);  unset($nomer_doski7);
                              setcookie ("nomer7", $nomer7, time() );  setcookie ("nomer_doski7", $nomer_doski7, time() );         }
                   }
...
if(!file_exists($sxx))   {  //echo 'файл '.$sxx.' присутствует';
                          ...
                          setcookie ("nomer_doski7", $nomer_doski7, time() + 1);
                          setcookie ("nomer7", $nomer7, time() + 1);
                     ?>
<script>location.reload();</script>                     
<?
// echo "<meta http-equiv='refresh' content='1;url=index.php?doska=$name&tip7=$tip7'>";
}
...
if($file = fopen($base_hod_ip7, "r")) {   $b4=0;} 
while(!feof($file)) { 
...
fclose($openchatdb);
 ...
 $base_hod_ip7 = 'ip7/hod/'.$name.'.txt';

$openchatdb=fopen($base_hod_ip7,"w");
 if(fwrite($openchatdb,"")) { // echo 'Записано в файл '.$base_hod_ip7.'</font><strong></center><br>';
                                           }
fclose($openchatdb);
...
if($_POST['sbros']) { 
//$game='5'; 
//$nomer7_rezet=''; $nomer7=$nomer7_rezet;
setcookie ("game", $game, time() + 4);
 setcookie ("nomer_doski7", $nomer_doski7, time() + 1);  
 setcookie ("nomer7", $nomer7, time() + 1);  
...
setcookie ("game", $game, time() + 4);
 setcookie ("nomer_doski7", $nomer_doski7, time() + 1);  
// setcookie ("nomer7", $nomer7,4 time() + 1);  
 setcookie ("timer", $timer, time() + 1);  
setcookie ("btc", $btc, time() + 1);  
$rrt='4'; 
...
if($nomer7>' ') {  setcookie ("nomer7", $nomer7, time() +3600);  }

//если нет своей игры,и открываю свою игру из оповещений
if(!$nomer_doski7 && $nomer7) { $nomer_doski7=$nomer7; setcookie ("nomer_doski7", $nomer_doski7, time() +3600*24*700); }

//echo '<br>$nomer7='.$nomer7;
