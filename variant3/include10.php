...
if($sopernik) { if($nomer3 || $nomer_doski3) { unset($nomer3);  unset($nomer_doski3);
                              setcookie ("nomer3", $nomer3, time() );  setcookie ("nomer_doski3", $nomer_doski3, time() );         }
                   }
...
if(!file_exists($sxx))   {  //echo 'файл '.$sxx.' присутствует';
                          ...
                          setcookie ("nomer_doski3", $nomer_doski3, time() + 1);
                          setcookie ("nomer3", $nomer3, time() + 1);
                     ?>
<script>location.reload();</script>                     
<?
// echo "<meta http-equiv='refresh' content='1;url=index.php?doska=$name&tip3=$tip3'>";
}
...
if($file = fopen($base_hod_ip3, "r")) {   $b4=0;} 
while(!feof($file)) { 
...
fclose($openchatdb);
 ...
 $base_hod_ip3 = 'ip3/hod/'.$name.'.txt';

$openchatdb=fopen($base_hod_ip3,"w");
 if(fwrite($openchatdb,"")) { // echo 'Записано в файл '.$base_hod_ip3.'</font><strong></center><br>';
                                           }
fclose($openchatdb);
...
if($_POST['sbros']) { 
//$game='5'; 
//$nomer3_rezet=''; $nomer3=$nomer3_rezet;
setcookie ("game", $game, time() + 4);
 setcookie ("nomer_doski3", $nomer_doski3, time() + 1);  
 setcookie ("nomer3", $nomer3, time() + 1);  
...
setcookie ("game", $game, time() + 4);
 setcookie ("nomer_doski3", $nomer_doski3, time() + 1);  
// setcookie ("nomer3", $nomer3,4 time() + 1);  
 setcookie ("timer", $timer, time() + 1);  
setcookie ("btc", $btc, time() + 1);  
$rrt='4'; 
...
if($nomer3>' ') {  setcookie ("nomer3", $nomer3, time() +3600);  }

//если нет своей игры,и открываю свою игру из оповещений
if(!$nomer_doski3 && $nomer3) { $nomer_doski3=$nomer3; setcookie ("nomer_doski3", $nomer_doski3, time() +3600*24*700); }

//echo '<br>$nomer3='.$nomer3;
