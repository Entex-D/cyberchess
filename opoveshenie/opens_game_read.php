...
$f='0';
$s='./archive_doski/'.$name.'/anketa/list_game.txt';

 $ChatDB1=file($s);
$ChatDB2='./archive_doski/'.$name.'/anketa/list_game.txt'; 
...
if($style=='16' || $style_run=='16') { $color='#FFE4B5';  $dd='color=#505050'; }
 ...
 if($status=='0') { $color='#606060';  $dd='color=#DCDCDC'; $status_text='Свободно'; }
 if($status=='2') { $color='#606060';  $dd='color=#DCDCDC'; $status_text='Отменена'; }
 if($status=='1') { $status_text='Занято'; }
 if($status=='3') { $color='#202020';  $dd='color=#DCDCDC';  $status_text='Завершено'; }
...
<td bgcolor='.$color.'><br><strong><font size=1 '.$dd.'><center>'.$igrok2.'</center></font></strong><br></td>
