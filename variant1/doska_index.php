...
if(!$nomer && isset($_COOKIE['nomer_doski'])) { //echo '<br>считан номер доски='.$nomer_doski;
  ...
  if($game!='5' && $game>'0' && !isset($_COOKIE['nomer_doski'])) {

   include('./ochki/vsego_igr_write.php'); 

$fx=rand(1,10000); $nomer_doski='a'.$fx.'a'; 
if(setcookie ("nomer_doski", $nomer_doski, time() + 3600*24*700)) { //echo '<br>куки не записались'; 
} 
                                                               }
...
fclose ($file);
              ...
              if($game<'5' && $game>'0' && !isset($_COOKIE['nomer_doski'])) { echo ''; 
...
if($name==$doska) { //$fx=rand(1,10000); $nomer_doski='n'.$fx.'r'; 
//if(!setcookie ("nomer_doski", $nomer_doski, time() + 3600*24*700)) { echo '<br>куки не записались'; } 

$adres='./archive_doski/'.$name.'/tip1/'.$nomer_doski;
...
//echo '<br>$doska='.$doska;
//echo '<br>$name='.$name;
//echo '<br>$nomer_doski='.$nomer_doski;

$figury='./archive_doski/'.$name.'/tip1/'.$nomer_doski.'/'.$nomer_doski.'.txt';
...
$dannye_archive='./archive_doski/'.$name.'/tip1/'.$nomer_doski.'/'.$nomer_doski.'_statix.txt';
...
$logi_text='<font size=1><em>'.date("d.m.Y|G:i:s ").'</em></font> Игрок <strong><em>'.$name.'</em></strong> создал  новую игру '.$game_g.' на доске '.$nomer_doski.' вариант #1 Ставка '.$dixa20.' btc';
...
if($game=='5' && isset($_COOKIE['nomer_doski'])) { echo ''; 

$logi='logi.txt';

$logi_text='<font size=1><em>'.date("d.m.Y|G:i:s ").'</em></font> Игрок <strong><em>'.$name.'</em></strong> отменил игру на доске '.$nomer_doski.' вариант #1';
$openchatdb=fopen($logi,"a");
 if(fwrite($openchatdb,$logi_text."\n")) { ;  }
fclose($openchatdb);
...
<? if($name==$exxo[4] && $nomer_doski && $exxo[5]=='0' && $exxo[18]=='0') { echo '';  ?>
...
if($exxo[1]!='Comp') { echo '<br><a href=index.php?doska='.$doska.'&tip=1&nomer_doski='.$nomer_doski.'&ilost=yes>Сдаться</a>'; } 
 }}
...
<? if($name==$exxo[3] && $nomer_doski && $exxo[5]=='0' && $exxo[18]=='0') {  // если играем белыми - доска в своем положении
?>
...
echo '<br><a href=index.php?doska='.$doska.'&tip=1&nomer_doski='.$nomer_doski.'&ilost=yes>Сдаться</a>'; 
 }}
// конец обновление вместо "онлайн людей" от 15.06.2020
?>
...
<a target=_blank href=model_game/variant1-igra1-style<? echo $style_run; ?>.jpg><img src=model_game/variant1-igra1-style<? echo $style_run; ?>.jpg width=30% /></a>

<? echo '<br><br><br>Создайте новую игру '.$name.'</center></strong></font></td>';
      //include('prosmotr_doski.php');
  } else if($nomer_doski && $name!=$exxo[3] && $name!=$exxo[4] && $exxo[18]=='0' && $exxo[1]!='Comp') {
 
      echo '<font size=4><strong><center>Доска '.$nomer_doski.' занята,создайте свою игру '.$name.'</center></strong></font>';
      echo '<font size=2><strong><em><center>
Посмотреть партию <a target=_blank href=variant1/archive_d_tip4_uvel.php?namer='.$doska.'&tip=1&nomer='.$nomer_doski.'>в архиве</a>
</center></em></strong></font></td>';
      //include('prosmotr_doski.php'); 

} else if($exxo[18]=='Отменена') {

     echo '<font size=4><strong><center>Доска '.$nomer_doski.' отменена,игроком '.$doska.'</center></strong></font>
               <font size=2><strong><em><center>Посмотреть доску '.$nomer_doski.' <a target=_blank href=variant1/archive_d_tip4_uvel.php?namer='.$doska.'&tip=1&nomer='.$nomer_doski.'>в архиве</a></center></em></strong></font>
</td>';
    //  include('prosmotr_doski.php'); 

if($exxo[5]!='0' || $exxo[18]!='0') {  
 $nomer_doski='';  setcookie ("nomer_doski", $nomer_doski, time() + 1);
$nomer='';  setcookie ("nomer", $nomer, time() + 1); 
}
...
if($exxo[5]==$name) {
     echo '<font size=4><strong><center>Вы выиграли играя '.$d.', '.$bex.' проиграл</center></strong></font>';
               ...
               echo '<font size=2><strong><em><center>Посмотреть доску '.$nomer_doski.' <a target=_blank href=variant1/archive_d_tip4_uvel.php?namer='.$doska.'&tip=1&nomer='.$nomer_doski.'>в архиве</a></center></em></strong></font>';
...
if($exxo[21]>'9' || $exxo[22]>'9') {
 include('./variant1/hod_vertical/hod_vertical.php'); 
                                                    }
}
...
$data1='./archive_doski/'.$doska.'/tip1/'.$nomer_doski.'/'.$nomer_doski.'_random_figura_white.txt';
...
if($exxo[21]>'9' || $exxo[22]>'9') {
 include('./variant1/random_figura/hod_vertical.php'); 
                                                    }
} 
 ...
 include('./variant1/hod_vertical/random_table.php'); 
...
if($zvuki_hody=='11') { 
 ...
 if($exxo[12]<2) { include('./variant1/zvuk_start_game.php'); } 
if($exxo[12]>1 && $exxo[12]!=11) { include('./variant1/zvuk.php'); }
